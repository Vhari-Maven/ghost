<script lang="ts">
  ...
  interface AlwaysDoByCategory {
    cardio: AlwaysDoItem[];
    core: AlwaysDoItem[];
    mobility: AlwaysDoItem[];
  }
  ...
  let {
    ...
    alwaysDoByCategory: AlwaysDoByCategory;
...
<Accordion title="Always Do Stack" icon="✅" defaultOpen={true}>
  <div class="space-y-4">
    {#each Object.entries(alwaysDoByCategory) as [category, items]}
      {@const catInfo = categoryLabels[category as keyof typeof categoryLabels]}
      <div>
        <h4 class="text-sm font-medium text-[var(--color-text-muted)] mb-2 flex items-center gap-2">
